    ...
    def calcServosDeg(self, target_pose):
        ...
        ## 各サーボの角度を計算する処理を記述
            self.__servos[0].setTargetDegree(
                math.degrees(math.atan2(target_pose[1], target_pose[0])) + self.__offsets_degree[0]
            )
        ...
        ## https://tajimarobotics.com/kinematics-two-link-model-2/ 
            ...
            self.__servos[1].setTargetDegree(math.degrees(rad_theta1) + self.__offsets_degree[1])
            self.__servos[2].setTargetDegree(math.degrees(rad_theta2) + self.__offsets_degree[2])
